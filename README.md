# NeurIPS25 - iOS App

NeurIPS25 is an iOS companion app for the NeurIPS 2025 conference (December 2-7, San Diego).

## Features

- 📅 Browse 200+ conference sessions
- 🎯 Create your personal agenda
- 📝 Review and rate papers
- 👥 Network with 2000+ researchers
- 📤 Export your schedule

## Download

Available on the iOS App Store (link coming soon)

## Support

### Contact
For support, questions, or feedback:
- **Email**: [triston.cao@gmail.com]
- **Issues**: Use the [Issues](https://github.com/tristonc/NeurIPS25_App/issues) tab above

### Common Questions

**Q: How do I export my schedule?**  
A: Go to My Agenda → Tap the export button → Choose Markdown or JSON

**Q: Is my data synced to the cloud?**  
A: No, all data is stored locally on your device for privacy.

**Q: Can I use this on iPad?**  
A: Not yet.

**Q: The app is free?**  
A: Yes, completely free with no ads or in-app purchases.

### Report Issues

Found a bug or have a feature request?
1. Go to [Issues](https://github.com/tristonc/NeurIPS25_App/issues)
2. Click "New Issue"
3. Describe the problem or suggestion

## Privacy

All data is stored locally on your device. No data is collected or transmitted to external servers.

[Privacy Policy](https://github.com/tristonc/NeurIPS25_App/blob/main/PRIVACY.md)

## About

Created by Triston Cao for the NeurIPS 2025 research community.

© 2025 Triston Cao
